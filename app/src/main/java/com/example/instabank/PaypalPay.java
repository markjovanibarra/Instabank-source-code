...
public class PaypalPay extends AppCompatActivity {
                ...
                usersRef.orderByChild("phoneNumber").equalTo(currentUserPhoneNumber)
                        .addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            public void onCancelled(@NonNull DatabaseError databaseError) {
                                Toast.makeText(PaypalPay.this, "Failed to fetch balance: " + databaseError.getMessage(), Toast.LENGTH_SHORT).show();
                            }
                        });
            }
        }
        ...
        buttonPay.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (amountString.isEmpty()) {
                    ...
                    if (amount < 10) {
                        // Show dialog for minimum amount requirement
                        showMinAmountDialog();
                        return;
                    } else if (amount > 50000) {
                        // Show dialog for maximum amount exceeded
                        showMaxAmountDialog();
                        return;
                    }
                }
                                ...
                                .addListenerForSingleValueEvent(new ValueEventListener() {
                                    ...
                                    public void onDataChange(DataSnapshot dataSnapshot) {
                                        if (dataSnapshot.exists()) {
                                                        ...
                                                        snapshot.getRef().child("balance").setValue(newBalance)
                                                                .addOnSuccessListener(new OnSuccessListener<Void>() {
                                                                    ...
                                                                    public void onSuccess(Void aVoid) {
                                                                        ...
                                                                        recordTransaction(name, accountNumber, -amount, "PayPal", referenceId);

                                                                        // Update balance on dashboard after successful payment
                                                                        updateDashboardBalance(newBalance);

                                                                        // Calculate and add rewards (2% of amount)
                                                                        double rewardAmount = amount * 0.001;
                                                                        addRewardsToUser(rewardAmount);

                                                                        Toast.makeText(PaypalPay.this, "Payment of ₱" + amount + " successful", Toast.LENGTH_SHORT).show();

                                                                        // Show the receipt
                                                                        showReceipt(name, accountNumber, amount, "PayPal", referenceId);
                                                                ...
                                                                .addOnFailureListener(new OnFailureListener() {
                                                                    ...
                                                                    public void onFailure(@NonNull Exception e) {
                                                                        Toast.makeText(PaypalPay.this, "Failed to deduct balance: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                                                                    }
                                                                });
                                                    } else {
                                                        // Show dialog for insufficient balance
                                                        AlertDialog.Builder builder = new AlertDialog.Builder(PaypalPay.this);
                                                        builder.setTitle("Insufficient Balance");
                                                        builder.setMessage("You have insufficient balance to complete this transaction.");
                                            ...
                                            Toast.makeText(PaypalPay.this, "User not found", Toast.LENGTH_SHORT).show();
                                        }
                                    }

                                    @Override
                                    public void onCancelled(DatabaseError databaseError) {
                                        Toast.makeText(PaypalPay.this, "Failed to deduct balance: " + databaseError.getMessage(), Toast.LENGTH_SHORT).show();
                                    }
                                });
                    }
                }
    ...
    private boolean isValidAmount(String amount) {
        // Check if amount contains only digits (no dots or symbols)
        String regex = "^[0-9]+$";
    ...
    private void showInvalidAmountDialog() {
        ...
        builder.setMessage("Amount should only contain digits (no dots or symbols).");
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
    ...
    private void showMinAmountDialog() {
        ...
        builder.setMessage("The minimum amount for a transaction is ₱10.");
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
    ...
    private void showMaxAmountDialog() {
        ...
        builder.setMessage("The maximum amount allowed for a transaction is ₱50,000.");
        builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
        ...
        usersRef.orderByChild("phoneNumber").equalTo(currentUser.getPhoneNumber())
                .addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        for (DataSnapshot snapshot : dataSnapshot.getChildren()) {
                            ...
                            snapshot.getRef().child("rewards").setValue(newRewards).addOnCompleteListener(task -> {
                                if (task.isSuccessful()) {
                                    Toast.makeText(PaypalPay.this, "Rewards updated successfully", Toast.LENGTH_SHORT).show();
                                } else {
                                    Toast.makeText(PaypalPay.this, "Failed to update rewards: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
                    }
                    ...
                    public void onCancelled(@NonNull DatabaseError error) {
                        Toast.makeText(PaypalPay.this, "Failed to update rewards: " + error.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
    }
}
